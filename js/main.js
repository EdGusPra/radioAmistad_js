document.addEventListener('DOMContentLoaded', () => {
	...
	const getTapa = async () => {
		...
		let artist =
			getInfo?.data.artist.replace(/\w\S*/g, (w) =>
				w.replace(/^\w/, (c) => c.toUpperCase())
			) ?? '♪ ♫ ♬ 𝅘𝅥𝅯 𝅘𝅥𝅰 𝅘𝅥𝅲';

		let title =
			getInfo?.data.title.replace(/\w\S*/g, (w) =>
				w.replace(/^\w/, (c) => c.toUpperCase())
			) ?? '♪ ♫ ♬ 𝅘𝅥𝅯 𝅘𝅥𝅰 𝅘𝅥𝅲';
		...
		const thumbs = await axios.get(
			`https://itunes.apple.com/search?term=${getInfo.data.artist}&media=music`
		);
		...
		arte.forEach((element, i) => {
			if (i <= 9) {
				...
				a.src =
					element ??
					'https://res.cloudinary.com/doutajybn/image/upload/v1626971342/radioamistad/portadaDisco.webp';
		...
		if (getTapa.data.artists) {
			tapa.src =
				getTapa.data.artists[0].strArtistThumb ??
				'https://res.cloudinary.com/doutajybn/image/upload/v1626971342/radioamistad/portadaDisco.webp';
		} else {
			tapa.src =
				'https://res.cloudinary.com/doutajybn/image/upload/v1626971342/radioamistad/portadaDisco.webp';
	...
	const setDate = () => {
		...
		document.querySelector(
			'.word'
		).innerHTML = `${hour}:${minsFormat}:${secondsFormat}`;
	};
